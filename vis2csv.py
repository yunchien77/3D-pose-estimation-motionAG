...
def show3Dpose(vals, ax):
    ...
    # ax.view_init(elev=0., azim=90)   # 正側面
    ...
    for i in np.arange( len(I) ):
        ...
        # ax.text(x[0], y[0], z[0], f'{i}', color='black', fontsize=5)
        # ax.text(x[1], y[1], z[1], f'{i}', color='red', fontsize=5)
...
def get_pose3D(video_path, output_dir):
    ...
    post_out_all_origin = []
    post_out_all_process = []

    for idx, clip in tqdm(enumerate(clips)):
        #print(f'idx: {idx}, clip: {clip}')
        ...
        post_out_all_origin.append(post_out_all)
        ...
        for j, post_out in enumerate(post_out_all):
            ...
            #post_out_all_process[j] = post_out
            post_out_all_process.append(post_out)
    ...
    print("post_out_all_origin")
    print(post_out_all_origin)
    print(len(post_out_all_origin))

    print("post_out_all_process")
    print(post_out_all_process)
    print(len(post_out_all_process))
    ...
    return post_out_all_origin, post_out_all_process
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--out', type=str, default='0.csv', help='input video')
    # parser.add_argument('--input_dir', default='bb', help='input video')
    # parser.add_argument('--output_dir', type=str, default='', help='input video')
    ...
    output_origin_name = 'keypoints_origin.csv'
    output_process_name = 'keypoints_process.csv'
    output_origin_csvpath = os.path.join(output_dir, output_origin_name)
    output_process_csvpath = os.path.join(output_dir, output_process_name)
    ...
    pose_all_o, pose_all_p = get_pose3D(video_path, output_dir)
    
    with open(output_origin_csvpath, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(header)

        for idx in range(len(pose_all_o)):
            for data in pose_all_o[idx]:
                write_row = []
                for joint_data in data:
                    for i in range(3):
                        write_row.append(joint_data[i])
                writer.writerow(write_row)

    
    with open(output_process_csvpath, 'w', newline='') as f:
        writer = csv.writer(f)
        writer.writerow(header)

        for idx in range(len(pose_all_p)):
            write_row=[]
            for joint_data in pose_all_p[idx]:
                for i in range(3):
                    write_row.append(joint_data[i])
            writer.writerow(write_row)
